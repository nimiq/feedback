...
<template>
  <label for="attachments" :class="{ 'cursor-pointer': files.length === 0 }" group w-full>
    <h3 text-neutral-800 f-text-xs f-mb-2xs nq-label>{{ t('attachmentUploader.title') }}</h3>

    <div grid="~ cols-[repeat(auto-fit,128px)]" w-full f-gap-sm>
      <div
        v-for="(preview, index) in previews" :key="preview"
        class="f-rounded-2xs stack outline-1.5 outline-neutral-200 size-128!" :class="[
      ...
      >
        <img
          ...
          h-full="!" f-rounded-2xs bg-neutral-100 max-h-128 object-contain
        >
        <button
          type="button" :aria-label="t('attachmentUploader.deleteImageLabel')" outline="1.5 offset--1.5 white/8"
          f-inset-r--3xs f-inset-t--3xs stack rounded-full bg-white shadow self-start justify-self-end f-size-2xs
          @click.stop.capture="removeFile(index)"
        >
          <div text-neutral f-text-3xs i-nimiq:cross />
        </button>
      </div>

      <div
        v-if="files.length < maxFiles"
        flex="~ col justify-center items-center" f-rounded-2xs size-128 aspect-square outline="1.5 neutral/15"
      >
        <div text="neutral-700 group-hocus:neutral-800" transition-colors f-text-lg i-nimiq:mountain-frame />
        <span font-semibold f-mt-2xs>{{ t('attachmentUploader.uploadHere') }}</span>
        <span text="f-xs neutral-800 center" f-mt-3xs f-px-3xs>{{ t('attachmentUploader.anyImageFormat') }}</span>
      </div>
    </div>
