  ...
  >
    <div grid="~ gap-16 justify-center cols-[repeat(auto-fit,128px)]" mx-auto w-full>
      <div v-for="(preview, index) in previews" :key="index" stack rounded-4 size-128 aspect-square outline=" 1.5 neutral-200">
        <img :src="preview" :alt="`Preview ${index + 1}`" rounded-4 bg-neutral-100 object-contain>
        <button
          type="button" aria-label="Delete image" outline="1.5 offset--1.5 white/8" stack rounded-full bg-white
          size-24 shadow self-start right--12 top--12 justify-self-end @click.stop="removeFile(index)"
        >
    ...
    <input id="attachments" ref="fileInput" type="file" name="attachments" accept="image/*" multiple sr-only @change="handleFileSelect">
  </label>
</template>
